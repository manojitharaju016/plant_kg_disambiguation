...
if run_from_scratch == "y":
    ...
    nodes_df["entity1type_original"] = nodes_df["entity1type"].copy()
    nodes_df["entity2type_original"] = nodes_df["entity2type"].copy()


    nodes_df["entity1type"] = nodes_df["entity1type"].map(mapping_df.set_index("id")["parent_node"]).fillna("other")
    nodes_df["entity2type"] = nodes_df["entity2type"].map(mapping_df.set_index("id")["parent_node"]).fillna("other")


    nodes_df["connection_type"] = "[" + nodes_df["entity1type"].astype(str) + "] " + nodes_df["edge"].astype(str) + " [" + nodes_df["entity2type"].astype(str) + "]"

    nodes_df = nodes_df.reset_index(drop=True)



    inputs =list(nodes_df["connection_type"].unique())
    ...
    batch_size = 45000
    query_dir = "data/embeddings/edge_embeddings/queries"
    ...
    batch_df.to_csv(f"data/embeddings/edge_embeddings/batch_ids_{timestamp}.csv", index=False)
...
#check if the variable batch_df exists
if not "batch_df" in locals():
    batch_df = pd.read_csv(f"data/embeddings/edge_embeddings/batch_ids_{timestamp}.csv")
...
output_dir = "data/embeddings/edge_embeddings/output"
...
parquet_output_path = os.path.join(output_dir, f"edge_embeddings_{timestamp}.parquet")
